...
export function useCart(){const [lines,setLines]=useState<CartLine[]>([]);useEffect(()=>{const sync=()=>setLines(read());sync();window.addEventListener("storage",sync);window.addEventListener("turbo-cart",sync);return()=>{window.removeEventListener("storage",sync);window.removeEventListener("turbo-cart",sync)}},[]);return {lines,count:lines.length,totalQuantity:lines.reduce((n,x)=>n+x.quantity,0),subtotal:lines.reduce((n,x)=>n+x.quantity*x.unitPrice,0)}}