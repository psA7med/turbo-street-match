      ...
      <div className="turbo-container grid h-16 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 lg:h-20 lg:grid-cols-[auto_minmax(20rem,1fr)_auto] lg:gap-8">
        <div className="flex items-center justify-self-start lg:hidden">
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side="right" className="w-[88%] max-w-sm"><SheetTitle className="text-start">القائمة</SheetTitle><nav className="mt-8 grid gap-1"><SheetNavLink label="البحث" to="/search" onClick={() => setMobileOpen(false)} />{links.map(([label,to]) => <SheetNavLink key={to} label={label} to={to} onClick={() => setMobileOpen(false)} />)}<SheetNavLink label="تجار الجملة" to="/wholesale" onClick={() => setMobileOpen(false)} className="py-4 text-lg font-semibold text-primary" />{accountState === "admin"&&<SheetNavLink label="لوحة الإدارة — طلبات الجملة" to="/admin/wholesale" onClick={() => setMobileOpen(false)} className="py-4 text-lg font-semibold text-primary" />}{accountState !== "guest"&&<SheetNavLink label="حسابي" to="/account" onClick={() => setMobileOpen(false)} className="py-4 text-lg font-semibold" />}</nav></SheetContent>
          </Sheet>
        </div>
        <div className="flex min-w-0 items-center gap-8 lg:contents">
           <Link to="/" aria-label="TURBO الصفحة الرئيسية" className="justify-self-center lg:order-first lg:justify-self-auto"><img src={darkLogo.url} alt="TURBO" className="h-8 w-24 object-contain sm:w-28 lg:h-11 lg:w-40" /></Link>
