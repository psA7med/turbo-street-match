...
export function ProductCard({product}:{product:CatalogProduct}){
  ...
  </Link><div className="mt-4"><Link to="/products/$slug" params={{slug:product.slug}} className="font-bold hover:text-primary">{product.name_ar}</Link><div className="mt-1 flex items-center justify-between"><span className="font-bold">{price===null?"السعر قريباً":money(price)}</span><div className="flex gap-1">{Array.from(new Set(variants.map(v=>v.color_value).filter(Boolean))).slice(0,4).map(c=><span key={c!} className="size-4 rounded-full border border-border" style={{backgroundColor:c!}} aria-label="لون متاح"/>)}</div></div></div>
  </article>;
}
